...
int main()
{
    ...
    bounds_for_aubo<<Eigen::Matrix<double,6,1>().setConstant(3.05),Eigen::Matrix<double,6,1>().setConstant(-3.05);

    size_t nearest_counter{0}, all_valid_counter{0},ik_counter{0};
    const size_t max_iterations = 1e6; const double zero_thresh = 1e-3;
    ...
    std::cout<<"exactly the same IK solution coverage rate: "<<(double)(nearest_counter)/(double)max_iterations<<std::endl;
    std::cout<<"exactly the same IK solution using all valid method coverage rate: "<<(double)(all_valid_counter)/(double)max_iterations<<std::endl;
    std::cout<<"IK solution (equals to fk) coverage rate: "<<(double)(ik_counter)/(double)max_iterations<<std::endl;
    std::cout << "the average nearest_time consumption of nearest ik method: " << 1e6 * nearest_time/max_iterations << "us\n";
    std::cout << "the average nearest_time consumption of all valid ik method: " << 1e6 * all_time/max_iterations << "us\n";
}
