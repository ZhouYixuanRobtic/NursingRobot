...
TEST(planning_interface_test,compute_joint_path_test)
{
    ...
    spinner.start();
    planner::PlanningInterface planningInterface(planner::LAZY_RRT_CONNECT,
                               "manipulator_i5",
    ...
    output_file.open("/home/xcy/validGoals.txt");
    std::vector<double> temp;temp.resize(6);
    while(++iter_index<=max_iterations){
       ...
       //auto start_valid = planningInterface.getRandomValidJointState();
        ...
        std::string s;
        getline(output_file,s);
        std::istringstream stringGet(s);
        stringGet>>temp[0]>>temp[1]>>temp[2]>>temp[3]>>temp[4]>>temp[5];
        auto goal_valid = state_space::JointSpace(temp);
