...
void test_no_collision_func(int dimensions = -1, const Eigen::MatrixX2d *bounds_ptr = nullptr)
{
    ...
    rrt_2d.constructPlan(planner::RRT_REQUEST<SPCIFIC_STATE>(start_state, goal_state, 50));
    std::vector<SPCIFIC_STATE, Eigen::aligned_allocator<SPCIFIC_STATE>> path;
    clock_t start(clock());
    if (rrt_2d.planning()) {
        clock_t end(clock());
...
void test_with_collision_func(int dimensions = -1, const Eigen::MatrixX2d *bounds_ptr = nullptr)
{
    ...
    rrt_2d.constructPlan(planner::RRT_REQUEST<SPCIFIC_STATE>(start_state, goal_state, 1e7));
    std::vector<SPCIFIC_STATE, Eigen::aligned_allocator<SPCIFIC_STATE>> path;
    if (rrt_2d.planning()) {
        clock_t end(clock());
        LOG(INFO) << "planning time consumption: " << static_cast<double>(end - start) / CLOCKS_PER_SEC;
