...
namespace planner {
    ...
    public:
        ...
        RRT(std::function<size_t(T)> hashT, std::size_t dimensions,
            std::function<T(double *)> arrayToT = NULL,
            std::function<void(T, double *)> TToArray = NULL)
            :_tree_ptr(std::make_shared<Tree<T,Distance>>(hashT,dimensions,arrayToT,TToArray)),
            _bounds_ptr(nullptr),
        ...
        {
            _goal = goalState;
            _tree_ptr->setGoalState(goalState);
        }
        ...
        virtual bool planning()
        {
            ...
            if ((!_is_step_relative && _isReached(_tree_ptr->RootVertex(),_tree_ptr->Goal()))
                || _is_step_relative && _d_min <= std::numeric_limits<double>::epsilon()) {
                _tree_ptr->addState(_tree_ptr->Goal(),_tree_ptr->RootVertex());
            ...
            for (int i = 0; i < _iter_max; ++i) {
                ...
                if (r < _goal_bias)
                    new_vertex = _steer(_tree_ptr,_tree_ptr->Goal(), nullptr, true);
                else
