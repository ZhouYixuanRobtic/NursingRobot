...
namespace kinematics{
    ...
    */
    ...
    protected:
         ...
         */
        bool _nIkInSpace(const state_space::SE_3 & desired_pose, state_space::JointSpace& joint_angles, double eomg, double ev);

        /**@brief computes numerical forward kinematics (Jacobian iterative method) with respect to body frame
         * @param eomg: error of omega (rotation)
         * @param ev: error of velocity (linear)
         */
        bool _nIkInBody(const state_space::SE_3 & desired_pose, state_space::JointSpace& joint_angles, double eomg, double ev);
    ...
    public:
        ...
        void setHomeConfiguration(const state_space::SE3& home_configuration){home_configuration_ = home_configuration;};
        ...
        IK_SINGULAR_CODE analyticalIkSolutions(Eigen::MatrixXd &joint_solutions, const state_space::SE_3 & desired_pose, const state_space::JointSpace* ik_references_ptr);

        /**@brief combine numerical method and analytical method
         * @return all valid IK solutions
         * @note only use with a reference
         * */
        bool allValidIkSolutions(Eigen::MatrixXd & joint_solutions, const state_space::SE_3 & desired_pose, const state_space::JointSpace* reference_ptr);
         ...
         * */
        bool nIk(const state_space::SE_3& desired_pose, state_space::JointSpace& joint_angles, double eomg=1e-7, double ev=5e-7)
        {
            if(joint_angles.size()== 0) throw std::invalid_argument("numerical ik method must have a reference, now it's empty");
